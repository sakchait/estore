...
const ShoppingCart = ({ isVisible, quantity, handleOnClose }: TProps) => {
  ...
  return (
    ...
    >
      ...
      >
        <div className="flex items-center justify-between py-3 border-b border-orange-300 mx-6">
          <h2 className="text-gray-800 text-xl font-light">Shopping Cart ({quantity})</h2>
          <Button onClick={handleOnClose} className="p-2 size-11 border-white hover:border-orange-300">
            <CloseIcon width={18} />
          </Button>
        </div>
        ...
        <div className="absolute bottom-0 left-0 right-0 h-[140px] bg-white border-t border-orange-300 flex flex-col items-center justify-center gap-4 mx-6">
          {!!cartItems?.length && (
          ...
          <Button
            ...
            className="text-gray-500 text-sm w-4/5 border-orange-300 bg-gray-100 hover:border-gray-400 hover:bg-gray-200 active:border-gray-500 active:bg-gray-300"
          >
            Back to Shop
          </Button>
        </div>
