...
const Button = ({ onClick, size = "md", type = "button", disabled = false, className, children }: TProps) => {
  ...
  return (
    <button
      ...
      type={type}
      className={cn(
        "flex cursor-pointer bg-white border border-orange-300 items-center justify-center px-4 py-2 transition-all gap-4 duration-300 text-gray-700 rounded-lg disabled:cursor-default hover:bg-gray-100 active:bg-gray-200 disabled:bg-gray-100",
        sizeClasses[size],
        className
      )}
      onClick={onClick}
